import React from "react";
import "./dashboard.css";
import { FaPhone, FaUsers, FaDollarSign, FaSearch } from "react-icons/fa";
import { MdLogout, MdEmail, MdChat, MdVideoCall } from "react-icons/md";
import { PieChart, Pie, Cell, Tooltip, Legend } from "recharts";

export default function Dashboard() {
  const dataKPI = [
    { name: "Chat", value: 25, color: "#2ecc71" },
    { name: "Email", value: 20, color: "#f1c40f" },
    { name: "Video", value: 10, color: "#e74c3c" },
    { name: "Điện thoại", value: 45, color: "#3498db" },
  ];

  const dataStatus = [
    { name: "Chờ xử lý", value: 5, color: "#e74c3c" },
    { name: "Đang xử lý", value: 10, color: "#f39c12" },
    { name: "Đã xử lý", value: 85, color: "#27ae60" },
  ];

  const calls = [
    {
      name: "Nguyễn Văn Anh",
      phone: "0901234567",
      time: "17:30:00 15/10/2024",
      duration: "00:05:23",
      agent: "Trần Thị Bình",
      status: "Đã xử lý",
    },
    {
      name: "Lê Thị Cúc",
      phone: "0907654321",
      time: "16:15:00 15/10/2024",
      duration: "00:03:45",
      agent: "Lê Văn Cường",
      status: "Đã xử lý",
    },
    {
      name: "Phạm Văn Đức",
      phone: "0909876543",
      time: "15:45:00 15/10/2024",
      duration: "00:08:12",
      agent: "Phạm Thị Dung",
      status: "Đã xử lý",
    },
  ];

  return (
    <div className="layout">
      
      {/* SIDEBAR */}
      <aside className="sidebar">
        <div className="logo">
          <div className="logo-ic">CC</div>
          <div>
            <h3>KienlongBank</h3>
            <p>TT.DVKH</p>
          </div>
        </div>

        <ul className="menu">
          <li className="active">Dashboard</li>
          <li>Cuộc gọi</li>
          <li>Khách hàng</li>
          <li>Giao dịch</li>
          <li><MdChat /> Chat</li>
          <li><MdEmail /> Email</li>
          <li><MdVideoCall /> Video Call</li>
          <li>CMS</li>
          <li>SLAs</li>
          <li>CRM Demo</li>
          <li>Báo cáo</li>
          <li>QA Scoring</li>
        </ul>

        <div className="user-box">
          <div className="avatar">NV</div>
          <div>
            <p className="name">Cao Thanh Admin</p>
            <p className="mail">admin@example.com</p>
          </div>
          <button className="settings">Cài đặt</button>
        </div>
      </aside>

      {/* MAIN */}
      <main className="main-area">

        {/* HEADER */}
        <header className="header">
          <div className="search-box">
            <FaSearch />
            <input
              placeholder="Tìm kiếm khách hàng theo CIF, CCCD, SĐT, email..."
            />
          </div>

          <div className="header-actions">
            <span className="status">Tư vấn viên đang làm việc: <b>24</b></span>
            <span className="user">admin</span>
            <span className="logout"><MdLogout /> Đăng xuất</span>
          </div>
        </header>

        {/* PAGE TITLE */}
        <h2 className="title">Dashboard TT.DVKH</h2>
        <p className="desc">Tổng quan về hoạt động tổng đài và KPI hằng ngày</p>

        {/* TOP KPIs */}
        <div className="kpi-top">
          <div className="kpi-item">
            <FaUsers />
            <div>
              <p className="kpi-title">Tổng Membership</p>
              <h4>156</h4>
              <span className="kpi-green">+12% so với tháng trước</span>
            </div>
          </div>

          <div className="kpi-item">
            <FaPhone />
            <div>
              <p className="kpi-title">Tổng Doanh số</p>
              <h4>12</h4>
              <span className="kpi-green">+8% so với tháng trước</span>
            </div>
          </div>

          <div className="kpi-item">
            <FaUsers />
            <div>
              <p className="kpi-title">Lượt đăng ký MBS</p>
              <h4>126K</h4>
              <span className="kpi-green">+15% so với tháng trước</span>
            </div>
          </div>

          <div className="kpi-item">
            <FaDollarSign />
            <div>
              <p className="kpi-title">Doanh thu tháng này</p>
              <h4>128M</h4>
              <span className="kpi-green">+23% so với tháng trước</span>
            </div>
          </div>
        </div>

        {/* CHART SECTION */}
        <div className="charts-row">

          {/* CHART 1 */}
          <div className="chart-card">
            <div className="chart-header">
              <h3>Tỷ lệ thanh toán KPI</h3>
              <select>
                <option>Online + Offline</option>
              </select>
            </div>

            <PieChart width={380} height={300}>
              <Pie
                data={dataKPI}
                cx="50%"
                cy="50%"
                outerRadius={100}
                dataKey="value"
                label
              >
                {dataKPI.map((item, i) => (
                  <Cell key={i} fill={item.color} />
                ))}
              </Pie>
              <Tooltip />
              <Legend />
            </PieChart>
          </div>

          {/* CHART 2 */}
          <div className="chart-card">
            <div className="chart-header">
              <h3>Trạng thái Duyệt</h3>
              <select>
                <option>Tất cả</option>
              </select>
            </div>

            <PieChart width={380} height={300}>
              <Pie
                data={dataStatus}
                cx="50%"
                cy="50%"
                outerRadius={100}
                dataKey="value"
                label
              >
                {dataStatus.map((item, i) => (
                  <Cell key={i} fill={item.color} />
                ))}
              </Pie>
              <Tooltip />
              <Legend />
            </PieChart>
          </div>
        </div>

        {/* KPI SMALL BOXES */}
        <div className="kpi-small-row">
          <div className="kpi-small">
            <h4>Tỷ lệ thanh toán KPI</h4>
            <h2>96.8%</h2>
            <span className="kpi-green">+2.5% so với tháng trước</span>
          </div>

          <div className="kpi-small">
            <h4>Giao dịch</h4>
            <h2>98.2%</h2>
            <span className="kpi-green">Trong mục tiêu</span>
          </div>

          <div className="kpi-small">
            <h4>Tỷ lệ thanh toán TKDMS</h4>
            <h2>92.5%</h2>
            <span className="kpi-green">+1.2% so với tuần trước</span>
          </div>
        </div>

        {/* RECENT CALLS */}
        <div className="table-card">
          <div className="table-head">
            <h3>Cuộc gọi gần đây</h3>
            <a href="#">Xem tất cả →</a>
          </div>

          <table>
            <thead>
              <tr>
                <th>Khách hàng</th>
                <th>Số điện thoại</th>
                <th>Thời gian</th>
                <th>Thời lượng</th>
                <th>Agent</th>
                <th>Trạng thái</th>
              </tr>
            </thead>

            <tbody>
              {calls.map((c, i) => (
                <tr key={i}>
                  <td>{c.name}</td>
                  <td>{c.phone}</td>
                  <td>{c.time}</td>
                  <td>{c.duration}</td>
                  <td>{c.agent}</td>
                  <td className="status-done">{c.status}</td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
      </main>
    </div>
  );
}
